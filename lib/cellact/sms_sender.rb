...
module Cellact
  ...
  class SmsSender
    ...
    def send_sms(message_text, phones, options = {})
      ...
      raise ArgumentError.new("Reply to number must be between 4 to 14 digits: #{options[:sender_number]}") if
        options[:sender_number].present? && !PhoneNumberUtils.valid_sender_number?(options[:sender_number])
